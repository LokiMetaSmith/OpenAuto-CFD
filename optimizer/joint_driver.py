...
class JointPhysicsDriver(PhysicsDriver):
    ...
    def _sync_assets(self):
        """
        Syncs STLs generated in the parent joint directory to the individual solver sub-directories.
        """
        import os
        import shutil
        parent_tri_surface = os.path.join(self.case_dir, "constant", "triSurface")
        if not os.path.exists(parent_tri_surface):
            return

        for driver in self.drivers:
            child_tri_surface = os.path.join(driver.case_dir, "constant", "triSurface")
            os.makedirs(child_tri_surface, exist_ok=True)
            for filename in os.listdir(parent_tri_surface):
                src = os.path.join(parent_tri_surface, filename)
                dst = os.path.join(child_tri_surface, filename)
    ...
    def run_meshing(self, log_file=None, **kwargs):
        ...
        """
        # Ensure that geometry generated by simulation_runner in the parent dir
        # is available to the individual solver directories.
        self._sync_assets()
